use objc2::foundation::{NSObject, NSArray, NSString};
use objc2::rc::{Id, Shared};
use objc2::runtime::Object;
use objc2::{declare_class, msg_send, msg_send_id, extern_methods, extern_class, ClassType};
            ...
            for url in urls {
                if let Some(url) = url.absolute_string() {
                    AppState::queue_event(EventWrapper::StaticEvent(
                        Event::OpenURL { url: url.to_string() }
                    ));
                }
            }
        }
